...
This file is the working guide for agents editing `facebookresearch-repos`.

## What this repo is

`facebookresearch-repos` is a single-page static catalog of public repositories
from Meta Research's `facebookresearch` GitHub organization. It follows a
...
- `cloudflare/worker.js`: proxy used for
  `https://glaubi.net/facebookresearch`.

## Catalog rules

Included repositories must be:

- Owned by the `facebookresearch` GitHub org.
- Non-forks and non-archived.
- More than 200 stars.
- Pushed within the last three calendar months.

The page contains six rough research clusters:

1. Vision, Video & 3D
2. Language, Speech & Multimodal
3. Agents, Reasoning & Reinforcement Learning
4. Embodied AI, AR & Robotics
5. ML Systems, Optimization & Core Libraries
6. Data, Evaluation & Science
...
That command queries GitHub for current `facebookresearch` repositories
